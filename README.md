...
Windows: test_config_win.ps1 and CheckTomcatConfigWin.ps1, written in PowerShell.

The Unix scripts support Tomcat 7.0, 8.5, and 9.0, while the Windows scripts support Tomcat 7.0, 8.5, 9.0, 10.0, and 10.1, ensuring consistent functionality across platforms where applicable.
Features

Automated Testing: test_config_unix.py/test_config_win.ps1 runs comprehensive tests across multiple password types and server configurations, modifying tomcat-users.xml and server.xml to simulate scenarios and validating with the respective checking script.
...
10.0/10.1 (Windows only): Supports SecretKeyCredentialHandler and NestedCredentialHandler, aligning with 9.0 features.

Best Compliance Configurations
The following table outlines the optimal configurations for achieving compliance with NIST 800-53 IA-5 and CIS Tomcat Benchmark for each Tomcat version, applicable to both Unix and Windows (where supported):
...
10.0/10.1
CredentialHandler: SecretKeyCredentialHandlerAlgorithm: PBKDF2WithHmacSHA512Iterations: ≥10,000Salt Length: ≥16 bytesKey Length: 256 bitsPassword Type: Salted_PBKDF2
Identical to 9.0. PBKDF2 is preferred; SHA-512 with MessageDigestCredentialHandler is also compliant. Supported on Windows only.


Recommended Password Hashes

SHA-256: With MessageDigestCredentialHandler, minimum 10,000 iterations, and 16-byte salt (8.5, 9.0, 10.0, 10.1).
SHA-512: With MessageDigestCredentialHandler, minimum 10,000 iterations, and 16-byte salt (8.5, 9.0, 10.0, 10.1).
PBKDF2: With SecretKeyCredentialHandler (PBKDF2WithHmacSHA512), minimum 10,000 iterations, and 16-byte salt (9.0, 10.0, 10.1).
...
Hashed_SHA512: 128-character hex (secure with proper handler, 8.5, 9.0, 10.0, 10.1).
Salted_MD5: 32-character hex with 16-character salt (insecure).
Salted_PBKDF2: 32-character hex with 16-character salt (secure with proper handler, 9.0, 10.0, 10.1 preferred).

Overview
This repository provides tools to audit Apache Tomcat configurations for secure password storage and compliance with industry standards. The Python scripts (test_config_unix.py, CheckTomcatConfigUnix.py) are designed for Unix systems (Linux/macOS), while the PowerShell scripts (test_config_win.ps1, CheckTomcatConfigWin.ps1) target Windows. The Unix scripts support Tomcat 7.0, 8.5, and 9.0, while the Windows scripts additionally support Tomcat 10.0 and 10.1, making them ideal for system administrators and security professionals managing cross-platform Tomcat deployments.
Prerequisites
Unix (Linux/macOS)
...
Apache Tomcat 7.0, 8.5, 9.0, 10.0, or 10.1 installed (e.g., C:\Program Files (x86)\Apache Software Foundation\Tomcat 9.0).
...
Clone or Download Repository:git clone <repository-url> /home/user/tomcat-audit
...
Set Permissions:chmod +x test_config_unix.py CheckTomcatConfigUnix.py tomcat_manager.sh


Install Tomcat (if not installed):Use the provided tomcat_manager.sh script to install Tomcat 7.0.100, 8.5, or 9:sudo ./tomcat_manager.sh install 9    # Install Tomcat 9
sudo ./tomcat_manager.sh install 8.5  # Install Tomcat 8.5
sudo ./tomcat_manager.sh install 7    # Install Tomcat 7.0.100

Note: tomcat_manager.sh does not currently support Tomcat 10.0 or 10.1. To install these versions, download them manually from https://tomcat.apache.org/ and follow the official installation instructions.Features:
...
Logs details to /tmp/TomcatManager.log.Uninstallation:

sudo ./tomcat_manager.sh uninstall

Troubleshooting Download Issues:

If the download fails (e.g., "Failed to download Tomcat archive"), check /tmp/TomcatManager.log for HTTP status codes:cat /tmp/TomcatManager.log | grep -i "HTTP/"


Manually download the Tomcat 7.0.100 archive:cd /tmp
wget https://archive.apache.org/dist/tomcat/tomcat-7/v7.0.100/bin/apache-tomcat-7.0.100.tar.gz

Or use curl:curl -O https://archive.apache.org/dist/tomcat/tomcat-7/v7.0.100/bin/apache-tomcat-7.0.100.tar.gz

Verify the checksum:sha512sum /tmp/apache-tomcat-7.0.100.tar.gz

Expected: 6f43ae2b3a29a628d8ab2504706f1e8974c5e2f6e7db84e3776e37e7ca83c77ae5d5993f3c6eb97e0ca6db37eb4a48b1b7e6ec0348e2d1c4a7b6e9f2d1c3a5b7e.Re-run the script:sudo ./tomcat_manager.sh install 7


Test network connectivity:ping -c 4 archive.apache.org
curl -I https://archive.apache.org/dist/tomcat/tomcat-7/v7.0.100/bin/apache-tomcat-7.0.100.tar.gz


If behind a proxy, set environment variables:export http_proxy=http://proxy.domain:8080
...
Install Python (if not present):sudo apt update && sudo apt install python3  # Ubuntu/Debian
sudo yum install python3                   # CentOS/RHEL


Verify Tomcat:Ensure Tomcat is installed and the conf directory is accessible:ls -l /opt/tomcat/conf/server.xml /opt/tomcat/conf/tomcat-users.xml



Windows

Clone or Download Repository:git clone <repository-url> C:\Users\Admin\Desktop\tomcat-audit
cd C:\Users\Admin\Desktop\tomcat-audit

Or save test_config_win.ps1 and CheckTomcatConfigWin.ps1 to C:\Users\Admin\Desktop.
Set Execution Policy (if needed):Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy RemoteSigned


Verify Tomcat:Ensure Tomcat 7.0, 8.5, 9.0, 10.0, or 10.1 is installed and the conf directory is accessible:dir "C:\Program Files (x86)\Apache Software Foundation\Tomcat 9.0\conf\server.xml"
...
Command:cd /home/user/tomcat-audit
sudo ./test_config_unix.py


Example Output (Tomcat 9.0):Starting tests for CheckTomcatConfigUnix.py...
...
Command:sudo ./CheckTomcatConfigUnix.py


Example Output (secure):Checking Apache Tomcat configuration security...
...
Windows: Automated Testing with test_config_win.ps1
Tests multiple password types and credential handlers, restoring original configurations.
...
Example Output (Tomcat 10.0):[2025-04-23 14:10:00] Detected Tomcat version 10.0 at C:\Program Files (x86)\Apache Software Foundation\Tomcat 10.0\conf
[2025-04-23 14:10:00] Running test: 10.0_SecretKeyCredentialHandler_PBKDF2_Salted_PBKDF2 for Tomcat 10.0
...
Tomcat 10.1: 42 tests (6 server configs × 7 password types).
...
Command:.\CheckTomcatConfigWin.ps1
...
Unix: Logs to ~/TestTomcatConfig.log:cat ~/TestTomcatConfig.log


Windows: Logs to %LOCALAPPDATA%\Temp\TestTomcatConfig.log (e.g., C:\Users\Admin\AppData\Local\Temp\TestTomcatConfig.log):Get-Content $env:LOCALAPPDATA\Temp\TestTomcatConfig.log
...
For Tomcat 9.0, 10.0, and 10.1, configure SecretKeyCredentialHandler with PBKDF2:
...
For Tomcat 8.5, use MessageDigestCredentialHandler with SHA-512:
<Realm className="org.apache.catalina.realm.UserDatabaseRealm" resourceName="UserDatabase">
...
For Tomcat 7.0, use MessageDigestCredentialHandler with SHA-256 (no salt/iterations):
<Realm className="org.apache.catalina.realm.UserDatabaseRealm" resourceName="UserDatabase">
  <CredentialHandler className="org.apache.catalina.realm.MessageDigestCredentialHandler"
                    algorithm="SHA-256"/>
</Realm>
