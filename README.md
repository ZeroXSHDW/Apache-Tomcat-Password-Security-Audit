...
Both sets audit Tomcat 7.0, 8.0, 8.5, 9.0, and 10.0, ensuring consistent functionality across platforms. The scripts `test_config_unix.py` and `test_config.ps1` form a test framework to verify that `CheckTomcatConfigUnix.py` and `CheckTomcatConfigWin.ps1` correctly identify password types and assess compliance by simulating various configurations. The `tomcat_manager.sh` script facilitates the installation of Tomcat versions to ensure the test framework accurately determines password types.

### Features
- **Test Framework**: `test_config_unix.py`/`test_config.ps1` runs comprehensive tests across multiple password types and server configurations, modifying `tomcat-users.xml` and `server.xml` to simulate scenarios and validating the functionality of `CheckTomcatConfigUnix.py`/`CheckTomcatConfigWin.ps1`.
...
- **8.0**: Supports `MessageDigestCredentialHandler` (MD5, SHA-1, SHA-256) and `NestedCredentialHandler`.
- **8.5**: Adds SHA-512 support to `MessageDigestCredentialHandler` and `NestedCredentialHandler`.
- **9.0**: Includes `SecretKeyCredentialHandler` for PBKDF2-based hashing.
- **10.0**: Supports `SecretKeyCredentialHandler` for PBKDF2-based hashing, similar to 9.0, with enhanced Java 17 compatibility.
...
| **8.0**        | **CredentialHandler**: `MessageDigestCredentialHandler`<br>**Algorithm**: SHA-256<br>**Iterations**: ≥10,000<br>**Salt Length**: ≥16 bytes<br>**Password Type**: Hashed_SHA256 | SHA-256 with salt and iterations ensures robust security. SHA-512 and PBKDF2 are not supported. Plaintext, MD5, and SHA-1 are non-compliant. |
...
| **10.0**       | **CredentialHandler**: `SecretKeyCredentialHandler`<br>**Algorithm**: PBKDF2WithHmacSHA512<br>**Iterations**: ≥10,000<br>**Salt Length**: ≥16 bytes<br>**Key Length**: 256 bits<br>**Password Type**: Salted_PBKDF2 | Identical to 9.0, with PBKDF2 offering the highest security. `MessageDigestCredentialHandler` with SHA-512 is also compliant. |

### Recommended Password Hashes
- **SHA-256**: With `MessageDigestCredentialHandler`, minimum 10,000 iterations, and 16-byte salt (8.0, 8.5, 9.0, 10.0).
- **SHA-512**: With `MessageDigestCredentialHandler`, minimum 10,000 iterations, and 16-byte salt (8.5, 9.0, 10.0).
- **PBKDF2**: With `SecretKeyCredentialHandler` (PBKDF2WithHmacSHA512), minimum 10,000 iterations, and 16-byte salt (9.0, 10.0).
...
- **Hashed_SHA512**: 128-character hex (secure with proper handler, 8.5, 9.0, 10.0 only).
- **Salted_MD5**: 32-character hex with 16-character salt (insecure).
- **Salted_PBKDF2**: 32-character hex with 16-character salt (secure with proper handler, 9.0, 10.0 preferred).

## Overview

This repository provides tools to audit Apache Tomcat configurations for secure password storage and compliance with industry standards. The Python scripts (`test_config_unix.py`, `CheckTomcatConfigUnix.py`) are designed for Unix systems (Linux/macOS), while the PowerShell scripts (`test_config.ps1`, `CheckTomcatConfigWin.ps1`) target Windows. The test framework (`test_config_unix.py`, `test_config.ps1`) ensures that the auditing scripts (`CheckTomcatConfigUnix.py`, `CheckTomcatConfigWin.ps1`) function correctly across Tomcat 7.0, 8.0, 8.5, 9.0, and 10.0, making them ideal for system administrators and security professionals managing cross-platform Tomcat deployments. The `tomcat_manager.sh` script supports the test framework by installing Tomcat versions to validate password type detection.

## Prerequisites

### Unix (Linux/macOS)
- Python 3.6 or later.
- Apache Tomcat 7.0, 8.0, 8.5, 9.0, or 10.0 installed (e.g., `/opt/tomcat`, `/var/lib/tomcat10`).
...
- Apache Tomcat 7.0, 8.0, 8.5, 9.0, or 10.0 installed (e.g., `C:\Program Files (x86)\Apache Software Foundation\Tomcat 10.0`).
...
3. **Install Tomcat (if not installed)**:
   Use the provided `tomcat_manager.sh` script to install Tomcat 7.0, 8.0, 8.5, 9.0, or 10.0 for use with the test framework to verify accurate password type detection:
   ```bash
   sudo ./tomcat_manager.sh install 10   # Install Tomcat 10.0
   sudo ./tomcat_manager.sh install 9    # Install Tomcat 9.0
   sudo ./tomcat_manager.sh install 8.5  # Install Tomcat 8.5
   sudo ./tomcat_manager.sh install 8.0  # Install Tomcat 8.0
   sudo ./tomcat_manager.sh install 7    # Install Tomcat 7.0
   ```
   **Features**:
   - Installs OpenJDK 17 (for 10.0), OpenJDK 11 (for 8.5, 9.0), or OpenJDK 8 (for 7.0, 8.0) with version-specific URLs.
   - Downloads Tomcat archives from `https://archive.apache.org` or fallback mirrors.
   - Verifies checksums to ensure download integrity (e.g., SHA512 for 10.0.27: `6f43ae2b3a29a628d8ab2504706f1e8974c5e2f6e7db84e3776e37e7ca83c77ae5d5993f3c6eb97e0ca6db37eb4a48b1b7e6ec0348e2d1c4a7b6e9f2d1c3a5b7e`).
   ...
   **Troubleshooting Download Issues**:
   - If the download fails (e.g., "Failed to download Tomcat archive"), check `/tmp/TomcatManager.log` for HTTP status codes:
     ...
     ```
   - Manually download the Tomcat archive (e.g., for 10.0.27):
     ```bash
     cd /tmp
     ...
     Verify the checksum:
     ```bash
     sha512sum /tmp/apache-tomcat-10.0.27.tar.gz
     ```
     ...
     Re-run the script:
     ```bash
     sudo ./tomcat_manager.sh install 10
     ```
   - Test network connectivity:
     ```bash
     ping -c 4 archive.apache.org
     curl -I https://archive.apache.org/dist/tomcat/tomcat-10/v10.0.27/bin/apache-tomcat-10.0.27.tar.gz
     ```
   - If behind a proxy, set environment variables:
     ...
     sudo -E ./tomcat_manager.sh install 10
     ```
   **Notes**:
   - Requires internet connectivity for downloading Tomcat and OpenJDK.
   - Uses `/opt/tomcat` as the installation directory.
   - Tomcat 7.0 and 8.0 require OpenJDK 8, which is installed manually if not found in repositories.
   - Tomcat 10.0 requires OpenJDK 17.
...
3. **Verify Tomcat**:
   Ensure Tomcat is installed and the `conf` directory is accessible:
   ```powershell
   dir "C:\Program Files (x86)\Apache Software Foundation\Tomcat 10.0\conf\server.xml"
   dir "C:\Program Files (x86)\Apache Software Foundation\Tomcat 10.0\conf\tomcat-users.xml"
   ```

## Usage

### Unix: Test Framework with `test_config_unix.py`
The test framework (`test_config_unix.py`) verifies that `CheckTomcatConfigUnix.py` correctly identifies password types and assesses compliance by testing multiple password types and credential handlers, restoring original configurations afterward.
...
- **Example Output** (Tomcat 10.0):
  ...
  Found Tomcat at /opt/tomcat/conf, version: 10.0
  Detected Tomcat version 10.0 at /opt/tomcat/conf
  Backed up original files to /tmp/TomcatConfigBackup
    Test: 10.0_SecretKeyCredentialHandler_PBKDF2_Salted_PBKDF2
      Description: Testing Salted_PBKDF2 password with SecretKeyCredentialHandler_PBKDF2 CredentialHandler
      Modified server.xml: Set SecretKeyCredentialHandler_PBKDF2
      Modified tomcat-users.xml: Set password for testuser to Salted_PBKDF2 (4b6f7e8c9d0a1b2c3d4e5f60718293a4:1234567890abcdef)
      Actual output:
...
- **Test Counts**:
  ...
  - Tomcat 8.0: 25 tests (5 server configs × 5 password types).
  - Tomcat 8.5: 35 tests (5 server configs × 7 password types).
  - Tomcat 9.0: 42 tests (6 server configs × 7 password types).
  - Tomcat 10.0: 42 tests (6 server configs × 7 password types).
...
### Windows: Test Framework with `test_config.ps1`
The test framework (`test_config.ps1`) verifies that `CheckTomcatConfigWin.ps1` correctly identifies password types and assesses compliance by testing multiple password types and credential handlers, restoring original configurations afterward.
...
- **Example Output** (Tomcat 10.0):
  ```
  [2025-04-22 14:10:00] Detected Tomcat version 10.0 at C:\Program Files (x86)\Apache Software Foundation\Tomcat 10.0\conf
  [2025-04-22 14:10:00] Running test: 10.0_SecretKeyCredentialHandler_PBKDF2_Salted_PBKDF2 for Tomcat 10.0
  Test output: - User 'testuser': Salted_PBKDF2 password (secure)
  ...
  [2025-04-22 14:10:05] All tests completed successfully
  ```
- **Test Counts**:
  - Tomcat 7.0: 15 tests.
  - Tomcat 8.0: 25 tests.
...
- **Example Output** (secure):
  ```
  Checking Apache Tomcat configuration security...
  Detected Tomcat version 10.0 at C:\Program Files (x86)\Apache Software Foundation\Tomcat 10.0\conf
  - User 'testuser': Salted_PBKDF2 password (secure)
    - Parameter: Password Type = Salted_PBKDF2 [PASS]
    - Parameter: CredentialHandler = org.apache.catalina.realm.SecretKeyCredentialHandler [PASS]
    - Parameter: Algorithm = PBKDF2WithHmacSHA512 [PASS]
...
- **Unix Paths**:
  - Tomcat 7.0: `/opt/tomcat/conf/tomcat-users.xml`
  - Tomcat 8.0: `/opt/tomcat/conf/tomcat-users.xml`
  - Tomcat 8.5: `/opt/tomcat/conf/tomcat-users.xml`
  - Tomcat 9.0: `/opt/tomcat/conf/tomcat-users.xml`
  - Tomcat 10.0: `/opt/tomcat/conf/tomcat-users.xml`
- **Windows Paths**:
  - Tomcat 7.0: `C:\Program Files (x86)\Apache Software Foundation\Tomcat 7.0\conf\tomcat-users.xml`
  - Tomcat 8.0: `C:\Program Files (x86)\Apache Software Foundation\Tomcat 8.0\conf\tomcat-users.xml`
  - Tomcat 8.5: `C:\Program Files (x86)\Apache Software Foundation\Tomcat 8.5\conf\tomcat-users.xml`
  - Tomcat 9.0: `C:\Program Files (x86)\Apache Software Foundation\Tomcat 9.0\conf\tomcat-users.xml`
  - Tomcat 10.0: `C:\Program Files (x86)\Apache Software Foundation\Tomcat 10.0\conf\tomcat-users.xml`
- **Example**:
  ```xml
  <tomcat-users>
