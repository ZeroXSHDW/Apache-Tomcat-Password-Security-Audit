...
| **10.0/10.1**  | **CredentialHandler**: `SecretKeyCredentialHandler`<br>**Algorithm**: PBKDF2WithHmacSHA512<br>**Iterations**: ≥10,000<br>**Salt Length**: ≥16 bytes<br>**Key Length**: 256 bits<br>**Password Type**: Salted_PBKDF2 | Identical to 9.0. PBKDF2 is preferred; SHA-512 with `MessageDigestCredentialHandler` is also compliant. Supported on Windows only. |
...
3. **Install Tomcat (if not installed)**:
   ...
   ```
   **Note**: `tomcat_manager.sh` does not currently support Tomcat 10.0 or 10.1. To install these versions, download them manually from `https://tomcat.apache.org/` and follow the official installation instructions.
   **Features**:
...
2. **Set Execution Policy** (if needed):
   ...
   ```
3. **Verify Tomcat**:
   Ensure Tomcat 7.0, 8.5, 9.0, 10.0, or 10.1 is installed and the `conf` directory is accessible:
   ```powershell
...
- **Example Output** (Tomcat 10.0):
  ```
  [2025-04-23 14:10:00] Detected Tomcat version 10.0 at C:\Program Files (x86)\Apache Software Foundation\Tomcat 10.0\conf
