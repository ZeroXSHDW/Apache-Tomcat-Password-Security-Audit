...
# Remotely updates Tomcat user password security configuration on Windows servers (1:1 logic with UpdateTomcatUserWin.ps1, remote pattern from Remote_CheckTomcatConfigWin.ps1)
    ...
    try {
        $updateResults = Invoke-Command -ComputerName $server -Credential $Credential -ScriptBlock {
            ...
            function Write-Log {
                ...
                $script:logMessages += "$timestamp - $Level - $Message"
            ...
            if (-not (Test-AdminRights)) { return $logMessages }

            $Version = Get-TomcatVersion -TomcatHome $TomcatHome
            if (-not $Version) { return $logMessages }
